...
function searchAjax(key = 0) {
	...
	$.get(base + '/home_visits/ajax/angka_kontak_bpjs',
		{
			'nama':                nama,
			'tanggal':             tanggal,
		...
		function (result, textStatus, jqXHR) {
			...
			if (data.length > 0) {
				for (var i = 0, len = data.length; i < len; i++) {
					...
					temp += data[i].sistolik + '/' + data[i].diastolik + ' mmHg'
					temp += '</td>';
					temp += '<td class="berat_badan">';
					temp += data[i].berat_badan
					temp += '</td>';
					temp += '<td class="action">';
					temp += '<a href="' + base +'/home_visits/' + data[i].id+ '/edit" class="btn btn-warning"><span aria-hidden="true" class="glyphicon glyphicon-edit"></span></button>'
					temp += '</td>';
					temp += '</tr>';
				}
			} else {
