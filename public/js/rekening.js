			...
			for (var i = 0; i < data.data.length; i++) {
				...
				if( data.data[i].pembayaran_asuransi_id ){
					...
					if ( auth_id == 28 ){
						...
						if ( $('#ignored').length ) {
							temp += '<form method="POST" class="form-method" action="' + base + '/rekening_bank/unignore/' + data.data[i].id + '" accept-charset="UTF-8" class="m-t" role="form" enctype="multipart/form-data" autocomplete="off">';
						} else {
						...
						if ( $('#ignored').length ) {
							temp += '<button class="btn btn-info btn-sm " onclick="return confirm_click(this); return false;" type="button"><i class="fas fa-eye"> </i> Unignore</button>';
						} else {
