						...
						<div class="table-responsive">
							<table class="table table-hover table-condensed table-bordered">
								<tbody>
									...
									<tr>
										<td>Nilai</td>
										<td>{{ App\Classes\Yoga::buatrp($rekening->nilai) }}</td>
									</tr>
									<tr>
										<td>Deskripsi</td>
								...
								<table class="table table-hover table-condensed table-bordered">
									<tbody>
										...
										<tr>
											...
											<td>{{ App\Classes\Yoga::buatrp($rekening->pembayaran_asuransi->pembayaran)  }}</td>
										</tr>
									</tbody>
								</table>
							</div>
					...
					<table class="table table-hover table-condensed table-bordered">
						...
						<tbody>
							@if($rekening->pembayaran_asuransi->piutang_dibayar->count() > 0)
								@foreach($rekening->pembayaran_asuransi->piutang_dibayar as $piutang)
									<tr>
										<td>{{ $piutang->periksa_id }}</td>
										<td>{{ $piutang->periksa->pasien->nama }}</td>
										<td>{{ $piutang->periksa->tanggal }}</td>
										<td class="text-right">{{ App\Classes\Yoga::buatrp( $piutang->periksa->tunai ) }}</td>
										<td class="text-right">{{ App\Classes\Yoga::buatrp( $piutang->periksa->piutang ) }}</td>
										<td class="text-right">{{ App\Classes\Yoga::buatrp( $piutang->pembayaran ) }}</td>
										<td class="text-right">{{ App\Classes\Yoga::buatrp( $piutang->periksa->piutang - $piutang->pembayaran ) }}</td>
									</tr>
								@endforeach
							@else
								<tr>
						...
						<tfoot>
							<tr>
								...
								<th class="text-right">{{ App\Classes\Yoga::buatrp( $total_tunai ) }}</th>
								<th class="text-right">{{ App\Classes\Yoga::buatrp( $total_piutang ) }}</th>
								<th class="text-right">{{ App\Classes\Yoga::buatrp( $total_sudah_dibayar ) }}</th>
								<th class="text-right">{{ App\Classes\Yoga::buatrp( $total_piutang - $total_sudah_dibayar  ) }}</th>
							</tr>
						</tfoot>
					</table>
				</div>
