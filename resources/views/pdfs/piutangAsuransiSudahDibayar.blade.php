			...
			@if(count($sudah_dibayars) > 0)
				@foreach($sudah_dibayars as $sudah)
					<tr>
						...
						<td> {{ $sudah->nama_pasien }} </td>
						<td class="text-right"> {{ App\Classes\Yoga::buatrp($sudah->tunai) }}</td>
						<td class="text-right"> {{ App\Classes\Yoga::buatrp($sudah->piutang) }}</td>
						<td class="text-right"> {{ App\Classes\Yoga::buatrp($sudah->sudah_dibayar) }}</td>
					</tr>
				@endforeach
			@else
				<tr>
