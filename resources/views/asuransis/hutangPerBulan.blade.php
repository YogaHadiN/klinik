...
<ol class="breadcrumb">
	  ...
	  <li class="active">
		  <strong>Hutang Asuransi bulan {{ App\Classes\Yoga::bulan( $bulan ) }} {{ $tahun }}</strong>
	  </li>
</ol>
@stop
@section('content') 
						...
						<div class="table-responsive">
							<table class="table table-condensed table-bordered">
											...
											<tr>
												...
												@endif
														<a href="{{ url('asuransis/' .$ar->id. '/piutangAsuransi/BelumDibayar/'. date('Y-m-01', strtotime($ar->tanggal)).'/' .date('Y-m-t', strtotime($ar->tanggal))) }}">
															{{App\Classes\Yoga::buatrp( $ar->hutang - $ar->sudah_dibayar ) }}
														</a>
													</td>
											</tr>
										@endforeach
								...
								<tfoot>
									<tr>
										<td></td>
										<td class="text-right"><h3>{{ App\Classes\Yoga::buatrp( $total_hutang ) }}</h3></td>
										<td class="text-right"><h3>{{ App\Classes\Yoga::buatrp( $total_sudah_dibayar ) }}</h3></td>
										<td class="text-right"><h3>{{ App\Classes\Yoga::buatrp( $total_hutang - $total_sudah_dibayar ) }}</h3></td>
									</tr>
								</tfoot>
							</table>
						</div>
