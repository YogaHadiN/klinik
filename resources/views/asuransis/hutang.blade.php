...
@section('title') 
{{ env("NAMA_KLINIK") }} | Hutang Asuransi
						...
						@if(($data_piutang) > 0)
							@foreach($data_piutang as $dp)
								<tr
									...
									<td class="text-right">{{ App\Classes\Yoga::buatrp( $dp->piutang ) }}</td>
									<td class="text-right">{{ App\Classes\Yoga::buatrp( $dp->sudah_dibayar ) }}</td>
									<td class="text-right">{{ App\Classes\Yoga::buatrp( $dp->piutang -  $dp->sudah_dibayar  ) }}</td>
