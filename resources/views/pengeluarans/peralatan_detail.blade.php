...
@section('title') 
{{ env("NAMA_KLINIK") }} | Detail Pembelian Peralatan
							...
							@if($fakturbelanja->belanjaPeralatan->count() > 0)
								@foreach($fakturbelanja->belanjaPeralatan as $f)
									<tr>
										...
										<td class="text-right">{{ App\Classes\Yoga::buatrp(  $f->harga_satuan  ) }}</td>
										<td class="text-right">{{ $f->jumlah }}</td>
										<td>{{ $f->masa_pakai }} tahun</td>
										<td class="text-right">{{App\Classes\Yoga::buatrp(  $f->penyusutan_total  )}} </td>
									</tr>
								@endforeach
							@else
								<tr>
