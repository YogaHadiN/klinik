		...
		<div class="panel-heading">
			<div class="panel-title">
				...
				<div class="panelRight">
					<h3> Saldo : {{ App\Classes\Yoga::buatrp( $saldo ) }}</h3>
				</div>
			</div>
		</div>
						...
						@if($gopays->count() > 0)
							@foreach( $gopays as $g )
								<tr>
									...
									<td class="text-right">
										@if( $g->menambah )
											{{ App\Classes\Yoga::buatrp( $g->nilai ) }}
										@else 
											0
										@endif
									</td>
									<td class="text-right">
										@if( $g->menambah )
											0
										@else 
											{{ App\Classes\Yoga::buatrp( $g->menambah ) }}
										@endif
									</td>
									...
									<td>
										@if( $g->pengeluaran->faktur_image )
											<a class="btn btn-success btn-xs" target="_blank" href="{{ url('img/belanja/lain/' . $g->pengeluaran->faktur_image) }}">struk</a>
										@else
