...
@section('title') 
{{ env("NAMA_KLINIK") }} | Laporan Dispensing Obat BPJS
						...
						<tbody>
							@if(count($array) > 0)
								@foreach( $array as $a )
									<tr>
										<td>{{ $a['nama_staf'] }}</td>
										<td class="text-right">{{ App\Classes\Yoga::buatrp( $a['jumlah_harga_beli'] ) }}</td>
										<td class="text-right">{{App\Classes\Yoga::buatrp($a['jumlah_harga_jual'])  }}</td>
										<td class="text-right">{{ $a['jumlah_pasien'] }}</td>
										<td class="text-right">{{App\Classes\Yoga::buatrp($a['dispensing_per_pasien'])}}</td>
									</tr>
								@endforeach
							@else
								<tr>
