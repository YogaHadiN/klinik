...
class LaporansController extends Controller
{
	...
	public function index() {
		...
		$jumlah_pasien_baru = $this->jumlahPasienBaru($bulanIni, $id_bulan_ini);

		$query  = "SELECT count(id) as jumlah_periksa ";
		$query .= "FROM periksas ";
		$query .= "WHERE tanggal like '{$bulanIni}%' ";
		$total_pasien = DB::select($query)[0]->jumlah_periksa;
		$jumlah_pasien_lama = $total_pasien - $jumlah_pasien_baru;
		if ($total_pasien > 0) {
			$persen_pasien_lama = round( $jumlah_pasien_lama / $total_pasien * 100 );
			$persen_pasien_baru = 100 - $persen_pasien_lama;
		} else {
			$persen_pasien_lama =0;
			$persen_pasien_baru = 0;
		}
		$umum = [];
		$kandungan = [];
		$gigi = [];
		...
		foreach ($aps as $ap) {
			if ($ap->poli == 'umum') {
				$umum[] = $ap;	
			} elseif( $ap->poli == 'kandungan'  ){
				$kandungan[] = $ap;	
			} elseif( $ap->poli == 'gigi'  ){
				$gigi[] = $ap;	
			} elseif( $ap->poli == 'darurat'  ){
				$darurat[] = $ap;	
			}
		}
		$staf          = Yoga::stafList();
	...
	*/
	...
	public function harian()
	{
		...
		$hariinis = $this->hariinis($tanggal, $asuransi_id);

		// return $rincian;
		$rincian = [];
		$sama = false;
		$polis = $this->polisHarian($periksas);
