...
class CustomController extends Controller
{
  ...
  public function __construct()
	...
	public function kembali2($id){
		...
		return redirect()->back()->withPesan(Yoga::suksesFlash('<strong>' . $pasien->id . ' - ' . $pasien->nama . ' </strong>Berhasil dikembalikan ke Ruang Periksa <strong>Poli ' . ucwords(strtolower($periksa->poli)) . '</strong>'));
	}
		...
		foreach ($transaksis as $k => $transaksi) {
			...
			foreach ($jenis_tarif->bahanHabisPakai as $key => $bhp) {
				...
				$dispensings[] = [
					...
					'dispensable_type' => 'App\TransaksiPeriksa',
					'tanggal'          => $periksa->tanggal,
					'created_at'       => $timestamp,
					'updated_at'       => $timestamp
				];
	...
	private function formKembali($periksa){
		...
		try {
			...
			GambarPeriksa::where('gambarable_type', 'App\Periksa')
