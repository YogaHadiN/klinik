...
class JurnalUmumsController extends Controller
{
	...
	public function postPeralatan(){
				...
				while ( $tanggalSatu < date('Y-m-01')) {
					...
					foreach ($aa as $a) {
						...
						$penyusutans[] = [
							...
							'susutable_id'            => $a['id'],
							'susutable_type'          => 'App\BelanjaPeralatan',
							'nilai'                   => $susut_ini,
					...
					JurnalUmum::where('jurnalable_type', 'App\RingkasanPenyusutan')
		...
		$confirm = false;
		foreach ($pg_ids as $p) {
			$confirm = JurnalUmum::where('jurnalable_type', 'App\Pengeluaran')
	...
	public function normalisasi(){
		$pesan = Yoga::suksesFlash('Jurnal Umum berhasil di Normalisasi');
		$count = JurnalUmum::whereNull('coa_id')->count();
		if ($count) {
			$pesan = Yoga::suksesFlash('Ada Coa yang belum disesuaikan. Hubungi superAdmin untuk di Cek pengeluaran nya');
		}
		return redirect('laporans')->withPesan($pesan);
	}
	public function queryKonfirmasi($coa_id){
		$jurnals = JurnalUmum::where('jurnalable_type', 'App\Pengeluaran')->where('coa_id',$coa_id)->get(['jurnalable_id']);
		$ids = [];
	...
	public function omset_pajak(){
		...
		$query .= "AND ju.jurnalable_type not like 'App\\\NotaJual' ";
