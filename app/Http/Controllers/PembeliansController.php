...
class PembeliansController extends Controller
{
	...
	public function store()
	{
		...
		$jurnals            = [];
		$rak_updates        = [];
		$last_dispensing_id = (int)Yoga::customId('App\Dispensing') - 1;
		...
		foreach ($data as $dt) {
			...
			$dispensings[] = [
				...
				'dispensable_type' => 'App\Pembelian',
	...
	public function show($faktur_belanja_id)
	{
		...
		$jurnalumums   = JurnalUmum::where('jurnalable_type', 'App\FakturBelanja')
	...
	public function update($id)
	{
		...
		try {
			...
			//cari jurnal diskon
			//
			$jurnalDiskon = JurnalUmum::where('jurnalable_type', 'App\FakturBelanja')
				->where('jurnalable_id', $faktur_belanja_id)
				->where('coa_id', 50204)
				->where('debit', 0)
				->first();
			...
			if (Yoga::clean(  Input::get('diskon')  ) > 0) {
				if ($jurnalDiskon) {
					...
					$jrnl->jurnalable_type = 'App\FakturBelanja';
				...
				JurnalUmum::where('jurnalable_type', 'App\FakturBelanja')
					->where('jurnalable_id', $faktur_belanja_id)
					->where('coa_id', 50204)
					->where('debit', 0)
					->delete();
